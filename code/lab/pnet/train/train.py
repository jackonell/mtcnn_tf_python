...
def cls_loss_and_acc(pred,label):
    ...
    clsloss = -(filter_label*tf.log(filter_pred+1e-10))
...
def read_batch_data_from_tfrecord():
    ...
    batch_images,batch_labels,batch_bbxs,batch_landmarks = tf.train.batch([image,label,bbx,landmark],batch_size=cfg.BATCH_SIZE,capacity=cfg.BATCH_SIZE,num_threads=4)
...
def train():
    ...
    _cls_loss,_bbx_loss,_landmark_loss,accuracy = train_net_wise(PNet,12,cfg.BATCH_SIZE)
